...
class MSLP(object):
    ...
    **kwargs: optional
    ...
    def write_cuts_to_df(self):
        """Write all cuts to a dataframe.
        ...
        for t in range(self.T - 1):
            m = self.models[t]
            if type(m) != list:
                dict_coeff = m.get_cut_coeffs_and_rhs()
                # dict_coeff.update({'stage': t})
                df_dict[(t,-1)] = pandas.DataFrame(
                    dict_coeff,
                    columns=[state.varName for state in m.states] + ["rhs"],# + ['stage'],
                )
                concat_list.append(df_dict[(t,-1)])
            else:
                for k, m in enumerate(m):
                    dict_coeff = m.get_cut_coeffs_and_rhs()
                    # dict_coeff.update({'stage': t})
                    df_dict[(t, k)] = pandas.DataFrame(
                        dict_coeff,
                        columns=[state.varName for state in m.states]
                        + ["rhs"], # + ['stage'],
                    )
                    concat_list.append(df_dict[(t,k)])
        df_concat = pd.concat(concat_list, axis=0)
        return df_dict, df_concat
    ...
    def read_cuts_from_array(self, cuts):
        """Read all cuts from an NumPy array.
        csv files takes the form of:
            x.varName | y.varName | rhs
            a         | b         | c
        which specifies cut:
        ...
        for t in range(self.T - 1):
            m = self.models[t]
            # Alle spalten bis auf die letzte, die Stage spalte
            if type(m) != list:
    ...
    def read_cuts_from_df(self, df):
        """Read all cuts from a pandas DataFrame.
        csv files takes the form of:
            x.varName | y.varName | rhs
            a         | b         | c
        which specifies cut:
